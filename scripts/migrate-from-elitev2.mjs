#!/usr/bin/env node
// One-off: seed this app's database from elite-v2's, taking the 18+ channel and
// leaving the main library where it is (it belongs to an app of its own).
//
...
const DEST =
  process.argv[3] ||
  path.join(process.env.DATA_DIR || "/app/data", "adshortis.db");
...
const tx = db.transaction(() => {
  ...
  // afterwards was a main-channel playlist and is left behind with its clips.
  copy(
    "short_playlists",
...
// Per-user upload keys still point into elite-v2's shared profile tree. Merging
// them into this app's own library is a second step, on disk as well as in the
// database — see scripts/merge-user-uploads.mjs, which must run before the app
// serves these rows.
const strays = db
  .prepare(
  ...
  .get().c;
if (strays > 0) {
  log(`${strays} clips still carry a per-user key — run merge-user-uploads.mjs`);
}
