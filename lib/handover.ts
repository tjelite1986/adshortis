...
/**
 * Hand a clip over to the main shorts library.
 *
 * In elite-v2 this was a channel move: both libraries were rows in one table, so
 * the clip changed a column and its files moved to the other tree. The two
 * halves are two applications now, and writing into another app's database from
 * here would make two apps owners of one schema.
 *
 * So the clip goes in the way every other clip goes in: dropped into that app's
 ...
 * apps for a few minutes — that importer runs every 5 minutes and its
 * transcoder every 3 — which is the same lag any drop into that folder has.
 *
 * HANDOVER_MAIN_DIR is the mounted drop folder. Unset means the feature is
 * simply not available (the menu row asks first), never a silent no-op.
 */

export function handoverConfigured(): boolean {
  return Boolean(process.env.HANDOVER_MAIN_DIR);
}
...
/**
 * The name the receiving importer will read the clip's metadata back out of.
 *
...
export function handOverToMain(
  shortId: number
): { ok: true } | { ok: false; error: string } {
  const dropDir = process.env.HANDOVER_MAIN_DIR;
  if (!dropDir) {
    return { ok: false, error: "The main library is not reachable from here." };
  }
  ...
  // The poster belongs to a row that no longer serves anything; the other app
  // makes its own. Best effort — a leftover .jpg costs nothing and the sweep finds it.
  if (short.poster_key) {
    try {
