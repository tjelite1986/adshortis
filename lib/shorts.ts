...
// request into except '18plus', and a request naming another channel is answered
// with the only one that exists rather than a 404 it cannot act on.
export const CHANNEL: ShortChannel = "18plus";
...
// with the wrong filter, a script pointed at another app's file). The whole app
// is the adult library, so there is no per-channel gate left to wrap: reaching
// any page here already means a signed-in session. Still async: every call site
// awaits it, and a second channel here would have to read a cookie again.
...
export interface FeedShort {
  ...
  // The 18+ sorting bucket. Every clip has one; a freshly imported clip sits in
  // 'uncategorized' until an admin sorts it.
...
export function getFeed(
  ...
  // Category scope: one of the 18+ buckets, or null for the whole library. Last
  // in the list because it arrived after the others — a named-argument object
  // would be the tidier shape, and is a refactor for every call site at once.
  ...
  const query = qb
    ...
    // Defense in depth, not a gate: this app is 18plus-only, so a row on another
    ...
    // below — means such a mistake shows up as a missing clip rather than as a
    // clip from another app's library quietly served from this one.
    .where("s.channel", "=", CHANNEL)
    // Category scope, when the view asks for one bucket.
