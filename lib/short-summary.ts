...
//
// Nothing is summarised unless SHORT_AI_SUMMARY_CHANNELS says so: this whole
// library is the adult channel, and sending it to a third-party API is a
...
// Which channels may be described by a vision model. There is no default: the
// whole library here is the adult channel, and sending it to a third-party API
// is a decision to make deliberately, not one to inherit from a fallback. Unset
// means the Analysis page reports zero pending and describes nothing — set
// SHORT_AI_SUMMARY_CHANNELS=18plus to turn it on.
function allowedChannels(): ShortChannel[] {
  const raw = process.env.SHORT_AI_SUMMARY_CHANNELS || "";
  return raw
    .split(",")
    .map((c) => c.trim())
