...
// docker2/compose/adshortis/docker-compose.yml); the defaults under DATA_DIR
// keep dev self-contained.
const DATA_DIR = process.env.DATA_DIR || path.join(process.cwd(), "data");

// Permanent per-user content home: <PROFILE_ROOT>/u_<user>/shorts/...
// In production this is NOT mounted: the per-user uploads that arrived with the
// migration were merged into the channel store (scripts/merge-user-uploads.mjs)
// so the library is one tree with one owner. The root stays in the code because
// isUploadKey() still routes a stray u_<user>/ key here — after the merge there
// are none, and a key that reappears should fail loudly rather than resolve into
// a neighbouring app's directory.
