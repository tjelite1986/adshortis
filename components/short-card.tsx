...
export default function ShortCard({
  ...
  // Viewer preference (3-dot menu): which genre the FEED shows — the same `cat`
  // filter the chips drive, so both stay in sync through the URL.
  ...
  const showOnlyGenre = (cat: string) => {
    ...
    router.push(cat === "all" ? "/" : `/?cat=${cat}`);
  };

  // Admin: set the clip's genre from the side dropdown (optimistic).
  ...
  // Owner/admin: hand the clip over to the main shorts library. It leaves this
  ...
  const handOver = async () => {
    ...
    setBusyAction(true);
    setCoverMsg("Handing over to the main library…");
    try {
  ...
  return (
    <section className="relative flex h-full w-full snap-start snap-always items-center justify-center bg-black">
      ...
      {/* Genre picker (admin): a pill next to the Thumbnail button with a
          dropdown — sorts the clip into its bucket in one tap. "Genre", not
          "category": categories are reserved for tag-like things. */}
          ...
          {tags.length > 0 && (
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              {(tagsExpanded ? tags : tags.slice(0, 3)).map((t) => (
                <Link
                  key={t}
                  href={`/tag/${encodeURIComponent(t.slice(1))}`}
      ...
      {showMore && (
        ...
        >
          ...
          >
            ...
            {/* Genre filter for the feed itself. Every clip here is on the one
                channel, so this is always offered — it is the viewer's own
                preference, not an admin control. */}
            <div className="my-1 border-t border-white/10" />
